            ...
            if ($result_mondes = $mysqli->query($query_mondes)) {
                ...
                while($row_mondes = $result_mondes->fetch_assoc()) {
                    ...
                    if ($result_champs = $mysqli->query($query_champs)) {
                        ...
                        $query_valeurs = "
                                SELECT `vc`.`pk_valeur_champ`, `vc`.`fk_parent`, ( 
                                    SELECT COUNT(*) 
                                    FROM `user_valeur_champ` AS `uvc` 
                                    WHERE `uvc`.`fk_client` = " . $_SESSION["client"] . " 
                                    AND `uvc`.`fk_monde` = " . $row_mondes["pk_monde"] . " 
                                ...
                                AS `droits_valeur_champ`, (
                                    SELECT COUNT(*) 
                                    FROM `valeur_champ` AS `vc2` 
                                    WHERE `vc2`.`fk_client` = " . $_SESSION["client"] . " 
                                    AND `vc2`.`fk_monde` = " . $row_mondes["pk_monde"] . " 
                                    AND `vc2`.`fk_champ` = `vc`.`fk_champ` 
                                    AND `vc2`.`fk_parent` = `vc`.`pk_valeur_champ`
                                ) AS `enfants_valeur_champ`
                                FROM `valeur_champ` AS `vc` 
                        ...
                        if ($result_valeurs = $mysqli->query($query_valeurs)) {
                            ...
                            while($row_valeurs = $result_valeurs->fetch_assoc()) {
                                ...
                                if ($_SESSION["niveau"] >= 20 or $row_valeurs["droits_valeur_champ"]) {
                                    ...
                                    if ($row_valeurs["fk_parent"] == 0) {
                                        $json_niveau0 .= ' "' . $row_valeurs["pk_valeur_champ"] . '": ["%%' . $row_valeurs["pk_valeur_champ"] . '%%"]';
                                    } else {
                                        if ($row_valeurs["enfants_valeur_champ"] == 0) {
                                            $json_niveau0 = str_replace('["%%' . $row_valeurs["fk_parent"] . '%%"]', $row_valeurs["pk_valeur_champ"] . " : {}", $json_niveau0);
                                        } else {
                                            $json_niveau0 = str_replace('["%%' . $row_valeurs["fk_parent"] . '%%"]', $row_valeurs["pk_valeur_champ"] . ' : ["%%' . $row_valeurs["pk_valeur_champ"] . '%%"]"', $json_niveau0);
                                        }
                                    }
                                }
                            }
                        ...
                        $json_cascade = str_replace('"%%VALEURS%%"', $json_niveau0, $json_cascade);
