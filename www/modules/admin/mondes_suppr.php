<!--LOCALISATION-->
<div id="mondes-suppr" class="admin">
    <h1>Supresion de mundo</h1>
    <p class="warning">Borrar un mundo es una accion irreversible y de alto impacto!</p>
    <div id="choix-mondes-suppr">
        <p>Elige un mundo
            <ul class="list-mondes" id="liste-mondes-suppr"></ul>
        </p>
    </div>
    <div id="action-mondes-suppr">
        <p id="tag-mondes-suppr">Mundo <b class="nom-monde"></b></p>
        <p id="bilan-mondes-suppr">Que quieres hacer con los documentos relacionados?</p>
        <div class="designer-option">
            <h1>Borrar</h1>
            <p>Borrar <i><b>definitivamente</b></i> los <b class="nb-docs-bilan"></b> documentos del mundo <b class="nom-monde"></b>, liberando <b class="space-bilan"></b> de espacio.</p>
        </div>
        <div class="designer-option">
            <h1>Declasificar</h1>
            <p>Los <b class="nb-docs-bilan"></b> documentos declasificados regresaran en tu fila de espera.</p>
        </div>
        <p id="bouton-mondes-suppr" class="boutons">Confirmar</p>
    </div>
    
</div>
